...
TBinario avlABinario(TAvl a){
    ...
    if (a != NULL){
        ...
        res->dato = crearInfo(raizAvl(a), 0.00);
...
//Funcion auxiliar que crea un nodo hoja
TBinario crearNodoHoja(nat i){
	
	TBinario b = new _rep_binario;
    b->dato = crearInfo(i, 0.00);
    b->der = b->izq = NULL;
    return b;
 };

void BinarioBalanceadoRec(TBinario &b, ArregloNats elems, int inicio, int fin){
	
	if (inicio <= fin){
		
		int medio = trunc((inicio + fin) / 2);
		b = crearNodoHoja(elems[medio]);
		BinarioBalanceadoRec(b->izq, elems, inicio, medio - 1);
		BinarioBalanceadoRec(b->der, elems, medio + 1, fin);
	}
};


/*
 Lo hice recursivo primero creo el binario "res" y luego uso la funcion recursiva que
 hace todo el resto.Tambien hice una funcion auxiliar que solo crea un nodo "hoja"
 para no tener que escribir tanto en la recursiva. 
 */
TBinario binarioBalanceado(ArregloNats elems, nat n){

    TBinario res = crearBinario();
    BinarioBalanceadoRec(res, elems, 0, n-1);
